...
| 参考数量 | 当前适配限制：2.0 系列 9 图 / 3 视频 / 3 音频，总计 10；2.5 30 图 / 10 视频 / 10 音频，总计 50。2.5 已实测 9 + 3 + 3；各上限尚未逐项实测 |
...
已完成 76 项离线测试、管理端实际浏览器检查及 GitHub Actions 的构建 / 部署 / 代理运行环境 / 公网健康检查。Mini、Fast 与 2.5 的已验证组合见模型目录，其余组合保留未实测标记。
