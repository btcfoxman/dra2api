...
| 项目 | 证据与结论 |
| --- | --- |
| Mini | CLI：5–12 秒，480p / 720p；重新预检确认：文生和带图请求的 4 秒 / 15 秒均报 E_SCHEMA，CLI 按型号统一校验；实际成功：5 秒 / 480p / 16:9；观察到单账号单 Mini 任务限制，默认并发 1 |
...
| 2.5 | 当前 CLI：4–30 秒，4 秒 / 30 秒的 --dry-run 均通过；公开工具页仍写 5–30 秒，网关按实际 CLI 修正为 4–30。已实测 5 秒 / 480p / 16:9，9 图 + 3 视频 + 3 音频；4 秒仅完成预检，未实际生成。CLI 出现 1080p 与网页冲突，本次不扩充分辨率 |
